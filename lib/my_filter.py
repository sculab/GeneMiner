...
def filter_paired_reads_single_thread(_dict, _kmer, setpSize, file_1, file_2, out_dir, get_rc=False, data_size='all'):
    t1, t2, reads_count = time.time(), 0, 0
    infile_1 = gzip.open(file_1, 'r') if file_1[-3:].lower() == ".gz" else open(file_1, 'rb')
    infile_2 = gzip.open(file_2, 'r') if file_2[-3:].lower() == ".gz" else open(file_2, 'rb')
    temp_rec1 = [infile_1.readline(), infile_1.readline(), infile_1.readline(), infile_1.readline()]
...
def filter_single_reads_single_thread(_dict, _kmer, setpSize, file_1, out_dir, get_rc=False, data_size='all'):
    t1, t2, reads_count = time.time(), 0, 0
    infile_1 = gzip.open(file_1, 'r') if file_1[-3:].lower() == ".gz" else open(file_1, 'rb')
    temp_rec1 = [infile_1.readline(), infile_1.readline(), infile_1.readline(), infile_1.readline()]
    data_size = str(data_size)
    ...
    for i in re_filter_dict:
        ...
        if current % thread == thread_id:
            ...
            while True:
                ...
                used_dict = gethashdict(ref_path, kmer, get_rc=True, pos=False,
                                        print_info=False)  # filter  re-filter 的策略都是 参考序列反向互补,reads不反向互补
                ...
                if size >= sharp_cutoff_threshold:
                    ...
                    print("gene:{} kmer:{} richness:{} size:{}KB".format(gene_name, kmer, richness,int(size / 1024)),flush=True, end='\r')
...
def my_filter_main(args):
    ...
    if not os.path.isdir(out_dir):
        os.mkdir(out_dir)
    hashdict = gethashdict(reference, wordsize, get_rc=True, pos=False, print_info=True)  # 注意reference做了反向互补
    print(" " * 50, flush=True, end='\r')  # 之前的信息比较长，冲刷掉
    ...
    if data1 and data2:
        if sys.platform in ("win32") and size >= ref_limit_size or thread == 1:
            ...
            filter_paired_reads_single_thread(hashdict, wordsize, step, data1, data2, out_dir, False, data_size)
            combine_flag = 0
        else:
            reads_length = get_reads_length(data1)
            thread_list = []
    ...
    t2 = time.time()
    print(" " * 50, flush=True, end='\r')  # 之前的信息比较长，冲刷掉
    # print("Filter time used: {}s".format(round((t2 - t1), 4)))
        ...
        if len(re_filter_dict) > 0:
            ...
            for t in range(min(thread, len(re_filter_dict))):  # 减少进程开销
                p = Process(target=do_re_filter_loop,
                            args=(
                                parameter_information, re_filtered_reads_whole_bp_dict, ref_length_dict, re_filter_dict,
                                t, min(thread, len(re_filter_dict))))
                thread_list.append(p)
            for t in thread_list:
                t.start()
            for t in thread_list:
...
def my_filter_main_internal(configuration_information):
    # 初始化操作
...
if __name__ == '__main__':
    ...
    pars.add_argument("-t", "--thread", metavar="", dest="thread", help="Thread", type=int, default=4)
