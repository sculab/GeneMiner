...
class Extract_reference_fasta():
    def __init__(self,configuration_information,type,out_dir_name,ref,soft_boundary,rf,gene_max_length=5000, gene_min_length=300):
    ...
    def collect_reference_information_from_gb(self, feature, gene_location, rec, strand_all, identifier):
    ...
    def extract_reference_from_gb(self):
        ...
        if flag == 1:
            ...
            for rec in SeqIO.parse(ref, "gb"):
                ...
                start_all = 0
                end_all = len(rec.seq)
                species = rec.annotations["organism"].replace(" ", "_")  # 物种名
                accession = rec.name.replace(" ", "_")  # 样本名/genbank id
                identifier = species + "_" + accession
            ...
            for i in new_gene_name_list:
                ...
                for j in Genes_information:
                    if j["gene_name"] == i and len(j["gene_sequence"]) >= gene_min_length and len(
                        ...
                        my_record = SeqRecord(seq=j["gene_sequence"], id=j["gene_name"],
                                              description=j["identifier"])
    ...
    def filter_refrence(self):
        ...
        if rf=="s1":
            ...
            files=get_files(ref)
            for file in files:
                get_specify_txt_reference(file,rf)
        else:
            pass
...
if __name__ == '__main__':
    ...
    rf=r"D:\Happy_life_and_work\scu\python\Gene_Miner\eeee6 重构版本  大修 identity\reference.txt"
    target=Extract_reference_fasta(configuration_information,"tfa",r"../new100",r"../example/ref_fasta",75,rf,5000,300)
    target.extract_reference_from_fasta()
    target.filter_refrence()

    # target=Extract_reference_fasta(configuration_information,"tfa","out",r"../ref.fa",75,rf,5000,300)
    # target.extract_reference_from_fasta()
    # target.filter_refrence()
