...
class CorePipeLine():
    ...
    def filter_pipeline(self):
        ...
        # '''
        # if data1 and data2:
        #     cmd='python "{0}" -1 "{1}" -2 "{2}" -o "{3}"  -r "{4}" -k1 {5} -step_length {6} -t {7} -d {8}'.format(filter_path,
        #          data1,data2,filtered_out_path,reference_database_path,k1,step_length,thread_number,data_size)
        #
        #     # print(cmd)
        #     run_command(cmd)
        # elif single:
        #     cmd='python "{0}" -s "{1}" -o "{2}"  -r "{3}" -k1 {4} -step_length {5} -t {6} -d {7}'.format(filter_path,
        #         single,filtered_out_path,reference_database_path,k1,step_length,thread_number,data_size)
        #     run_command(cmd)
        # else:
        #     pass
    ...
    def assemble_pipeline(self):
        ...
        assemble_configuration_information={
            "thread_number":thread_number,"k2":k2,"out_dir":assembled_out_path,
            ...
            "change_times":change_seed,
            "scaffold_or_not":scaffold_or_not
        }
        my_assemble_main_internal(assemble_configuration_information)


        '''
        外挂版本
        '''
        # cmd='python "{0}" -i "{1}" -r "{2}" -o "{3}" -k2 {4} -limit_count {5} -limit_min_length {6}  -limit_max_length {7}  -change_seed {8}  -scaffold {9}  -t {10}  '.format(assemble_path,
        #     filtered_out_path,reference_database_path,assembled_out_path,k2,limit_count,limit_min_length,limit_max_length, change_seed,scaffold_or_not,thread_number)
        # run_command(cmd)
